...
class FaqViewModel extends ValueNotifier<FaqState> {
  ...
  Future<void> getFaqs(String businessId) async {
    ...
    var response =
        await _repository.getFaqs(businessId, true); // fetch from cache

    if (response.isSuccessful) {
      value = value.copyWith(isLoading: true, faqs: response.body);
    }

    response = await _repository.getFaqs(businessId, false);
