...
class ApiProvider{
  Future<List<GetUserChatModel>?> getUserChatAPI(
      GetUserChatRefreshEvent event, Emitter<GetUserChatState> emit
      ) async {
    ...
    try {
      ...
      if(response.statusCode == 200){
        ...
        List<GetUserChatModel> model = jsonResponse.map((json) => GetUserChatModel.fromJson(json)).toList();
        emit(GetUserChatSuccessState(model));
        return model;
      }else{
        emit(GetUserChatErrorState(response.statusMessage??""));
        List<dynamic> jsonResponse = response.data;
        return jsonResponse.map((json) => GetUserChatModel.fromJson(json)).toList();
      }
  ...
  Future<NewChatModel?> newChatAPI(
      NewChatRefreshEvent event, Emitter<NewChatState> emit
      ) async {
    emit(NewChatLoadingState());
    try {
      final client = await ApiClient.dioClient();
      Response response = await client.post("api/chat/new-chat",
        data: {
  ...
  Future<SendChatModel?> sendChatAPI(
      SendChatRefreshEvent event, Emitter<SendChatState> emit
      ) async {
    emit(SendChatLoadingState());
    try {
      final client = await ApiClient.dioClient();
      Response response = await client.post("api/chat/send",
        data: {
          "chatId": event.chatId,
          "businessId": StringHelper.businessId,
          "channel": event.channel,
