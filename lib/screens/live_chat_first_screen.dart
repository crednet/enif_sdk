...
class LiveChatFirstScreen extends StatefulWidget {
  ...
  @override
  State<LiveChatFirstScreen> createState() => _LiveChatFirstScreenState();
}

class _LiveChatFirstScreenState extends State<LiveChatFirstScreen> {
      ...
      appBar: CustomAppbar(
        ...
        onTap: () {
          Navigator.of(context).pushAndRemoveUntil(
              MaterialPageRoute(builder: (context) => const EnifHome()),
            ...
            Padding(
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return messages?[index].role == "assistance"
                        ? flexibleReceiver(index: index, list: messages)
                        : flexibleSender(index: index, list: messages);
                  }),
            ),
            140.h,
          ],
  ...
  Widget flexibleSender({required int index, List<MessagesData>? list}) {
    return Padding(
  ...
  Widget flexibleReceiver({required int index, List<MessagesData>? list}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 25),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.end,
  ...
  Future? functionCall(Map<String, String> map) {
    messages?.add(MessagesData.fromJson(map));
    Future.delayed(const Duration(milliseconds: 500), () {
      _scrollDown();
    });
    setState(() {});
